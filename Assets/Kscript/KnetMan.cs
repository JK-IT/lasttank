...
public class KnetMan : NetworkManager
{
    ...
    public override void OnServerChangeScene(string newSceneName) {
        Debug.Log( $"{this.name} -- ON-SERVER-CHANGE-SCENE this is called before Scenemanager.loadsceneasync" );
    }
    ...
    public override void OnClientChangeScene(string newSceneName, SceneOperation sceneOperation, bool customHandling) {
        Debug.Log( $"{this.name} -- ON-CLIENT-CHANGE-SCENE is called" );
    }
    ...
    public override void OnServerConnect(NetworkConnection conn) {
        //adding connection connlist
        if (connlist.Contains( conn ))
        {
            Debug.LogWarning( $"<color=lightblue>{this.name}Conn already exist : {conn.connectionId}</color>" );
            return;
        }
    ...
    {
        if( !(connlist.Remove(conn) && clientlist.Remove(conn)) )
        {
            Debug.Log( $"<color=red>{this.name} ---====---- CANNOT REMOVE CONN FROM LIST - conn {conn.connectionId} || list length : {connlist.Count} && {clientlist.Count}</color>" );
        }else
        {
            Debug.Log( $"<color=yellow>{this.name} --- Remove conn from list success - length: {connlist.Count}  && {clientlist.Count}</color>" );
        }
